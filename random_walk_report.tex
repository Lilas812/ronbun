...
$W=2$ の窓は直近の $1$ 本（再基準化して $2$ 点）しか含まないため，
評価関数を最小にするのは「確率 $1$ で直前と同じ方向へ進む」パラメータであり，
予測は $x_{n+1}^{\ast}\approx x_n+\gamma\,b_n$（本実験の格子では $\gamma=0.645$）という，
順張り予測を係数 $\gamma$ で縮小した形に帰着する。
すなわち，$W$ のスイープで最も誤差が小さかった設定は，
本モデルが備えるはずの局面推定の機能を最も使わない設定であった。

第二に，その $W=2$ の予測でさえ，
「直前と同じ方向へそのまま $1$ 進む」とする素朴な順張り予測には及ばなかった。
順張り予測は反転（確率 $\hat{r}$）のときだけ誤差 $2$ を出すから，
その MAE は $2\hat{r}=0.761$ であり，
本モデルの最良値 $0.847$（表\,\ref{tab:w_sweep}，$W=2$）よりも小さい。
持続性のある系列（$\hat{r}<1/2$）では，係数 $\gamma$ による縮小はかえって誤差を増やすため，
本モデルは誤差最小の設定でも順張り予測を上回れない。

第三に，「参照期間は短いほどよい」という傾向も，あらゆる局面で成り立つ法則ではない。
局面別の分析（\ref{subsec:patterns}節）では，
方向の持続が乏しいレンジ主体の局面で $W=2$ の優位はほとんど失われており，
この傾向は主として転換を含む局面によってもたらされていた。
すなわち，本実験の系列が転換の多い系列であったからこそ，短い $W$ が有利になった。
加えて本研究の結果は単一の資産・期間・値幅 $B$ に基づくものであり，
条件を変えても $W=2$ が最良であるとは限らない。
