...
\]
で定める。以下では，この $A$ をコイン行列ともよぶ。これに対して
...
\]
を仮定する。ここで $\mathbf{Z}=\{\ldots,-2,-1,0,1,2,\ldots\}$ は整数全体の集合である。
...
順序によらず二項係数 $\binom{n}{l}$ と $p^l q^m$ の積で表される。
...
相関付きランダムウォークについても，第3章の中心極限定理に対応する極限定理が成り立つ。
以下では，非負整数全体の集合を
$\mathbf{Z}_{\geq 0}=\{0,1,2,3,\ldots\}$
と表し，次のコイン行列で定まる相関付きランダムウォークを考える。
\begin{equation}
...
\begin{equation}
  \frac{\tilde{S}_n}{\sqrt{n}} \;\Rightarrow\; Y
  \label{eq:clt_corr}
\end{equation}
が成立する。
ここで $Y$ は正規分布 $N\!\left(0,\,\dfrac{a}{1-a}\right)$ に従う確率変数であり，
$N(m,\sigma^2)$ は平均 $m$ で分散 $\sigma^2$ の正規分布を表す。
\end{theorem}

なお，極限の確率変数 $Y$ の分布は初期確率ベクトル $\varphi$ に依存しないことに注意する。
...
実測値の下に取り残され，
天井を過ぎて下降へ転じる局面では逆に上昇の傾きを引きずって実測値の上方へ乖離した。
...
各パターンについて，$W=2,\,10,\,60$ の予測を実測値と同一図に重ねて示す。
