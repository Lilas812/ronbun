...
$W=2$ では再基準化した窓が $(0,\,b_n)$ の $2$ 点のみからなり，
評価関数を最小にするのは「確率 $1$ で直前と同じ方向へ進む」
パラメータの組すべてである。
予測値はそれら複数解の平均（式\eqref{eq:model_prediction_multiple}）として
$x_{n+1}^{\ast}\approx x_n+\gamma\,b_n$
（$0<\gamma<1$，本実験の格子では $\gamma=0.645$）の形に帰着する。
これは，「直前と同じ方向へやや控えめに進む」と読める，
順張り予測を係数 $\gamma$ で縮小したものにほかならない。
すなわち，$W$ のスイープで最も誤差が小さかった設定は，
本モデルが備えるはずの局面推定の機能を最も使わない設定であった。

第二に，その $W=2$ の予測でさえ，
「直前と同じ方向へそのまま $1$ 進む」とする素朴な順張り予測には及ばなかった。
順張り予測は継続で誤差 $0$，反転で誤差 $2$ を出すから，
その MAE は反転率から $2\hat{r}=0.761$ と定まる。
これは本モデルの最良値 $0.847$（表\,\ref{tab:w_sweep}，$W=2$）よりも小さい。
